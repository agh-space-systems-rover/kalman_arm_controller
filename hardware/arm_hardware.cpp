...
{
  std::lock_guard<std::mutex> lock(this->can_driver.m_read);
  ...
  {
    {
      ...
      std::lock_guard<std::mutex> lock(this->can_driver.m_write);
      for (int i = 0; i < 4; i++)
      {
        CAN_vars::joints[i].moveSetpoint.position_deg = joint_position_command_[i] * 180.0f / M_PI;
        CAN_vars::joints[i].moveSetpoint.velocity_deg_s = joint_velocities_command_[i] * 180.0f / M_PI;
    ...
    writer = std::async(std::launch::async, [&] { this->can_driver.write(current_control_type); });
  }
